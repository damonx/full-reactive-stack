...
public class QuoteReactiveControllerIntegrationTest {
	...
	@Before
	public void setUp() {
		this.webClient = WebClient.create("http://localhost:" + this.serverPort);
		this.quoteFlux = Flux.just(
	...
	public void simpleGetRequest() {
		// given
		given(this.quoteMongoReactiveRepository.findAll()).willReturn(this.quoteFlux);

		// when
		final Flux<Quote> receivedFlux = this.webClient.get().uri("/quotes-reactive").accept(MediaType.TEXT_EVENT_STREAM)
	...
	public void pagedGetRequest() {
		...
		given(this.quoteMongoReactiveRepository.retrieveAllQuotesPaged(PageRequest.of(1, 2)))
				.willReturn(this.quoteFlux.take(2));

		// when
		final Flux<Quote> receivedFlux = this.webClient.get().uri("/quotes-reactive-paged?page=1&size=2")
